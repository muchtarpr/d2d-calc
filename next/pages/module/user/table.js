...
export default class extends React.Component {
	...
	componentDidMount() {
		...
		$('#table-users').DataTable({
			...
			ajax: {
				type: 'post',
				url: snapConfig.BASE_API_URL + '/user/datatable',
			},
			responsive: true,
			language: {
	...
	actionDelete() {
		...
		axios({
			url: snapConfig.BASE_API_URL + '/user/delete',
			method: 'POST',
			data: {
				id: self.state.idDel
			},
	...
	actionDeleteAll() {
		...
		axios({
			url: snapConfig.BASE_API_URL + '/user/multi-delete',
			method: 'POST',
			data: {
			...
			<React.Fragment>
				<div className="card">
					<div className="card-body table-responsive">
						<table id="table-users" className="table responsive text-nowrap w-100">
							<thead>
								<tr>
