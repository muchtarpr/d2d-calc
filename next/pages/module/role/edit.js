...
export default class extends React.Component {
	...
	componentDidMount() {
		...
		axios({
			url: snapConfig.BASE_API_URL + '/role/edit',
			method: 'POST',
			data: {
				id: self.state.id,
			},
			...
			if (response.data.code == '2000') {
				let moduleList = self.moduleGrouped(response.data.data.files);

				self.setState({
					edit: response.data.data.role,
	...
	moduleGrouped(datas) {
		...
		for (let item of items) {
			...
			for (let itemChild of itemChilds) {
				...
				if (itemChildComponent && itemChildComponent != '#') {
					...
					let subMenu = _lo.find(datas, {
						component: itemChild.component,
					});
					if (subMenu) {
						itemParents.push(subMenu);
						modules.push(subMenu);
					} else {
						itemParents.push({
							component: itemChild.component,
							create: 0,
		...
		for (let data of datas) {
			if (data) {
				let isExistMenu = _lo.find(modules, {
					component: data.component,
				});

				if (!isExistMenu) {
					itemOthers.push([data]);
				}
			}
		}
	...
	handleSubmit(e) {
		...
		axios({
			url: snapConfig.BASE_API_URL + '/role/update/' + self.state.id,
			method: 'POST',
			data: data,
			timeout: snapConfig.TIMEOUT,
		})
